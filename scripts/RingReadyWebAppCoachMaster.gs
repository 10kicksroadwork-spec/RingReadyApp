 ...
 * WebExtract() / Web App Extract groups Athlete Raw Data by athlete after import.
 * Clears removed workouts from Athlete Raw Data when the PWA sends workout_completion_clear.
 *
 * Already running an older copy of this receiver? You can instead add
 * scripts/RingReadyAthleteRawSortPatch.gs for the sort-only drop-in.
 */
...
function rrImportPwaReceiverToAthleteRawData() {
  ...
  for (var r = 1; r < values.length; r++) {
    ...
    try {
      ...
      // Defer sheet sort until the full extract finishes (avoid O(n^2) resorts).
      if (rrAppendPwaWorkoutToAthleteRawData_(ss, payload, receivedAt, { skipSort: true })) imported++;
      else skipped++;
    } catch (err) {
      errors++;
  ...
  rrSortAthleteRawDataByAthlete_(rrEnsureAthleteRawDataSheet_(ss));

  SpreadsheetApp.getUi().alert(
    'Ring Ready PWA import complete',
    'Imported: ' + imported + '\nAlready present/skipped: ' + skipped + '\nErrors: ' + errors
      + '\nAthlete Raw Data sorted by Athlete.',
    SpreadsheetApp.getUi().ButtonSet.OK
  );
}
...
/**
 * Optional menu/action: regroup Athlete Raw Data by athlete without re-importing.
 * Safe to run anytime; preserves within-athlete row order for equal names.
 */
function rrSortAthleteRawDataByAthlete() {
  var sh = rrEnsureAthleteRawDataSheet_(rrSpreadsheet_());
  rrSortAthleteRawDataByAthlete_(sh);
  SpreadsheetApp.getUi().alert(
    'Athlete Raw Data sorted',
    'Workouts are now grouped by Athlete (column A).',
    SpreadsheetApp.getUi().ButtonSet.OK
  );
}

/**
 * Group Athlete Raw Data rows by athlete name.
 * Uses sheet Range.sort so cell formatting/colors move with each row.
 * Equal athlete names keep their prior relative order (week chronology).
 *
...
function rrSortAthleteRawDataByAthlete_(sh) {
  ...
  var athleteCol = rrFindAthleteColumnIndex_(headers);
  if (athleteCol < 0) return;

  // getRange(row, column, numRows, numColumns) — cover all data rows.
...
/** @param {string[]} headers @return {number} 0-based index, or -1 */
function rrFindAthleteColumnIndex_(headers) {
...
function rrAppendPwaWorkoutToAthleteRawData_(ss, payload, receivedAt, options) {
  if (payload.eventType !== 'sprint_session' && payload.eventType !== 'mile_test'
